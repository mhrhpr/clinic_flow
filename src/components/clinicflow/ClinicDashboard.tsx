...
function Patients() {
  ...
  const filtered = useMemo(() => patients.filter(p => [p.name,p.phone,p.service].join(" ").toLowerCase().includes(query.toLowerCase())), [query]);
...
function Appointments() {
  ...
  return <div className="space-y-6"><header><p className="text-sm font-semibold text-[var(--brand)]">Scheduling</p><h1 className="mt-1 text-3xl font-bold">Appointments</h1><p className="mt-2 text-sm text-[var(--muted)]">Keep the front desk, providers and patient flow synchronized.</p></header><div className="grid gap-4 sm:grid-cols-3"><Metric label="Today" value="18" note="4 remaining" /><Metric label="Confirmed" value="14" note="78% of schedule" /><Metric label="Waiting" value="2" note="Needs attention" /></div><section className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-5 shadow-sm"><div className="divide-y divide-[var(--border)]">{items.map((a,i) => <div key={a.time} className="flex flex-wrap items-center justify-between gap-4 py-4"><div className="flex items-center gap-5"><b className="w-14">{a.time}</b><div><p className="font-medium">{a.patient}</p><p className="mt-1 text-xs text-[var(--muted)]">{a.service} · {a.doctor}</p></div></div><select value={a.status} onChange={e => setItems(xs => xs.map((x,j) => j===i ? {...x,status:e.target.value} : x))} className="rounded-lg border border-[var(--border)] px-3 py-2 text-xs"><option>Confirmed</option><option>Waiting</option><option>Completed</option></select></div>)}</div></section></div>;
}
...
function PatientForm({onClose}:{onClose:()=>void}) { const [name,setName]=useState(""); return <div className="fixed inset-0 z-50 grid place-items-center bg-black/30 p-4"><div className="w-full max-w-md rounded-2xl bg-[var(--surface)] p-6 shadow-xl"><h2 className="text-lg font-semibold">New patient</h2><p className="mt-1 text-xs text-[var(--muted)]">Demo workflow; persistence is the next service integration gate.</p><input value={name} onChange={e=>setName(e.target.value)} placeholder="Full name" className="mt-5 w-full rounded-xl border border-[var(--border)] px-4 py-3 text-sm outline-none focus:border-[var(--brand)]" /><div className="mt-5 flex justify-end gap-2"><button onClick={onClose} className="rounded-xl border border-[var(--border)] px-4 py-2 text-sm">Cancel</button><button disabled={!name.trim()} onClick={onClose} className="rounded-xl bg-[var(--brand)] px-4 py-2 text-sm font-semibold text-white disabled:opacity-40">Create patient</button></div></div></div>; }
